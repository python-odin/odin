...
class TestFieldProxyDescriptor:
    @pytest.fixture
    def proxy_obj(self):
        class Target:
...
class TestResourceProxyOptions:
...
class TestResourceProxy:
    ...
    def test_proxy__iterable(self):
        actual = [
            Book(title="1984"),
            Book(title="1985 - The man strikes back"),
            Book(title="1986 - Return of the man"),
        ]
        target = BookProxy.proxy(actual)

        assert isinstance(target, proxy.ResourceProxyIter)
        assert ["1984", "1985 - The man strikes back", "1986 - Return of the man"] == [
            t.title for t in target
        ]
    ...
    def test_create_resource_from_dict(self):
        actual = create_resource_from_dict(
            {"title": "1984", "isbn": "1234567", "num_pages": 1}, BookProxy
        )
