...
class TestFields:
    def assertResourceEqual(self, first, second):
        ...
        if first._meta.resource_name != second._meta.resource_name:
            raise AssertionError(
                "Resources are different types %s != %s"
        ...
        for f in first._meta.fields:
            ...
            if first_val != second_val:
                raise AssertionError(
                    "Resource field %s does not match '%s' != '%s'"
                    % (f.name, first_val, second_val)
                )
    ...
    def test_arrayof_1(self):
        ...
        self.assertResourceListEqual(
            [ExampleResource(name="foo")], f.clean([{"name": "foo"}])
        )
        self.assertResourceListEqual(
            [ExampleResource(name="foo")],
            f.clean(
    ...
    def test_dictof_1(self):
        ...
        self.assertResourceDictEqual(
            {"foo": ExampleResource(name="foo")}, f.clean({"foo": {"name": "foo"}})
        )
        self.assertResourceDictEqual(
